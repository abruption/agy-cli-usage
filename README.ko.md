...
Node.js 22.13.0 이상이 필요합니다. Node 18/20 사용자는 설치 전에 Node를 업그레이드하세요. 타입 정의는 Node 22에 맞춥니다. CI는 최소 22.13.0 및 Linux·macOS·Windows의 Node 22/24를 검증합니다. 이 조합은 필수 `supported-tests` 체크로 검증하며 기존 Node 18/20 job은 제거했습니다. 완료된 전환 기록과 관리자의 복구 절차는 `.github/CI_MIGRATION.md`에 있습니다.

### CI·배포 패키지 검증

`npm run smoke:package`는 빌드·패키징 후 임시 경로에 설치하여 자격증명 없이 두 실행 별칭을 검증합니다. 한국어 문서도 배포 파일에 포함합니다. CI는 Action SHA 고정·읽기 전용 권한·의존성 감사·Linux/Windows 설치 검증을 적용합니다. 릴리스 생성과 npm 배포 job을 분리하고 배포 job만 OIDC 권한을 받으며 새 릴리스 태그를 체크아웃합니다. 필수 체크는 `lint`, `security-audit`, `package-smoke`, `supported-tests`입니다.

### PTY 인증

PTY 폴백은 같은 OS 세션에서 `agy` 로그인이 완료되어 있어야 합니다. 브라우저 인증 화면이면 로그인 필요 오류를 반환하며 인증 URL·코드는 출력하지 않습니다. API 자격증명을 읽을 수 있어도 agy 자체가 새 대화형 로그인을 요구할 수 있습니다. SSH와 로컬 터미널은 키체인 접근 조건이 다를 수 있습니다. PTY를 실행하는 세션의 자격증명 접근을 확인하세요. 해당 인증 화면이 장비 전체의 로그아웃을 뜻하지는 않습니다.
